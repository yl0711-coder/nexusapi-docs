...
<Tip>
現在のモデル可用性はモデルマーケットプレイスと対応マトリクスで確認します。
</Tip>
...
| フィールド | 必須 | 用途 |
| --- | --- | --- |
| `model` | はい | 現在の Key に見え、`supported_endpoint_types` に `openai` を含む正確なモデル ID |
...
`temperature`、ツール呼び出し、画像入力、構造化出力などのオプションは、「OpenAI 互換」だけで全モデルに保証されるものではありません。モデルの現在の可用性と対応能力は[API とモデル対応マトリクス](/ja/developer/capability-matrix)とモデルマーケットプレイスで確認してください。
...
`status_code=500, not implemented` が出た場合は、通常の 500 として高頻度に再試行しないでください。通常はモデルとリクエストプロトコルの不一致を示します。`supported_endpoint_types` に `openai` が含まれるか確認して、対応するガイドを使用してください。Request ID を保存して[自己確認](/ja/faq/self-check)を行います。
