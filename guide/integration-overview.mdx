...
仅当模型的 `supported_endpoint_types` 包含 `openai-response` 时，才使用 `/v1/responses`。若收到 `status_code=500, not implemented`，表示当前协议不匹配；请改用该模型标明支持的接口。Claude Code 使用原生 Messages 配置。
</Warning>
