...
| Codex CLI 等明确要求 Responses 的工具 | `/v1/responses` | 仅在 `supported_endpoint_types` 包含 `openai-response` 时使用 |
| Claude Code 或原生 Anthropic 客户端 | `/v1/messages` | 仅在 `supported_endpoint_types` 包含 `anthropic` 时使用；客户端会根据其配置补全路径 |
| 图片生成 | `/v1/images/generations` | 只适用于支持图片的模型 |

<Warning>
**不要把 Responses 当作通用替代接口**

如果模型的 `supported_endpoint_types` 未包含 `openai-response`，却通过 `/v1/responses` 收到 `status_code=500, not implemented`，这是协议不匹配。请改用该模型标明支持的接口；Claude Code 使用原生 Messages 配置。不要原样重试。
</Warning>
...
先阅读[接口与模型能力矩阵](/developer/capability-matrix)确认当前模型该用的协议；Chat Completions 的最小请求见[Chat Completions 接口](/developer/openai-compatible)。不同模型的可选字段请以所用客户端协议和控制台模型广场为准。

本帮助中心负责解释“如何选择和使用”，并给出可直接验证的最小请求。
