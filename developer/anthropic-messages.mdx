...
此接口只适用于 `supported_endpoint_types` 包含 `anthropic` 的模型。OpenAI 兼容应用优先使用 [Chat Completions](/developer/openai-compatible)；若模型未列出 `anthropic`，请改用其支持的接口。`not implemented` 通常表示协议不兼容，而不是临时错误。
</Warning>
...
| 自建 OpenAI SDK 程序 | 使用 [Chat Completions](/developer/openai-compatible) |
