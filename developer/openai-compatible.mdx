...
| 字段 | 是否需要 | 用途 |
| --- | --- | --- |
| `model` | 是 | 使用当前 Key 可见、且标注为 `openai` 的准确模型 ID |
...
`temperature`、工具调用、视觉输入、结构化输出等可选字段并不因“OpenAI 兼容”而对所有模型自动可用。字段 Schema 看 [Apifox 接口参考](https://nexusapi.apifox.cn/)，模型和渠道能力仍以[接口与模型能力矩阵](/developer/capability-matrix)及模型广场为准。
