...
| --- | --- | --- |
| `model` | Да | Точный ID, видимый текущему key, у которого `supported_endpoint_types` содержит `openai` |
...
`temperature`, tool calls, визуальный ввод и структурированный вывод не гарантированы для всех моделей только из-за «OpenAI-совместимости». Текущую доступность модели и поддерживаемые возможности проверяйте в [матрице возможностей](/ru/developer/capability-matrix) и каталоге.
...
При `status_code=500, not implemented` не повторяйте запрос как обычную ошибку 500 с высокой частотой. Обычно это означает несовместимость модели и протокола запроса. Проверьте, содержит ли `supported_endpoint_types` значение `openai`, затем используйте подходящее руководство. Сохраните Request ID и пройдите [самопроверку](/ru/faq/self-check).
