...
高级协议选项的位置；图中的 URL、模型和网关名称仅为界面示例，NexusAPI 的实际参数以本页下方 Claude / Codex 配置表为准。
...
| 字段 | 建议填写 |
| --- | --- |
| App / 应用 | `Claude Code`（旧版可能显示为 `Claude`） |
...
1. 新版 CC Switch 请选 `Claude Code`；若旧版界面显示 `Claude`，请选择对应 Claude Code 的 Provider，不要选 Claude Desktop。
2. Claude Code 的 NexusAPI Endpoint 不要加 `/v1`。
3. 配置完成后，在 CC Switch 中切换到该 Provider。
4. 重新打开终端，再运行 `claude`。
...
| API Key | `sk-您的 NexusAPI 令牌` |
| Primary Model / 主模型 | 当前令牌可用，且 `supported_endpoint_types` 包含 `openai-response` 的模型 ID |
...
1. Codex 使用 Responses 协议，Endpoint 需要带 `/v1`。
...
1. Claude Code 选择 `supported_endpoint_types` 包含 `anthropic` 的模型。
2. Codex 选择 `supported_endpoint_types` 包含 `openai-response`、且当前 API Key 可见的模型。
3. 仅选择当前 API Key 可用的模型。
