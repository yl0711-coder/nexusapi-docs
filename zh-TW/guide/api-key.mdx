...
| 憑證 | 用途 |
| --- | --- |
| API Key | 呼叫 Chat Completions、Responses、Messages、Images 與 `/v1/models` 等模型 API。 |
| 控制台登入狀態／Cookie | 僅供瀏覽器登入控制台，不能作為模型 API 的呼叫憑證。 |

請勿把任何憑證放進 URL 查詢參數，也不要向客服傳送完整憑證。
