...
- API Key：在 [NexusAPI 控制台](https://nexusapi.link)的令牌管理建立，通常以 `sk-` 開頭
...
請不要把真實 Key 寫入程式碼或提交到儲存庫。本頁範例僅適用於該模型的 `supported_endpoint_types` 包含 `openai` 時；僅在包含 `openai-response` 時使用 [Responses 介面與 Codex](/zh-TW/developer/responses)。圖片生成使用不同端點，請看[圖片模型說明](/zh-TW/guide/image-generation)；可選參數是否可用以模型廣場為準。
