...
請不要把真實 Key 寫入程式碼或提交到儲存庫。本頁範例僅適用於該模型的 `supported_endpoint_types` 包含 `openai` 時；若未包含 `openai-response`，不要把 `/v1/responses` 當作通用替代介面。圖片生成使用不同端點，請看[圖片模型說明](/zh-TW/guide/image-generation)；可選參數是否可用以模型廣場為準。
