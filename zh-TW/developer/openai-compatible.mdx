...
| 欄位 | 是否需要 | 用途 |
| --- | --- | --- |
| `model` | 是 | 使用目前 Key 可見，且 `supported_endpoint_types` 包含 `openai` 的正確模型 ID |
...
`temperature`、工具呼叫、視覺輸入、結構化輸出等選填欄位，不會因為「OpenAI 相容」而自動適用所有模型；模型目前支援的能力以[介面與模型能力矩陣](/zh-TW/developer/capability-matrix)及模型廣場為準。
...
若出現 `status_code=500, not implemented`，請勿將它視為一般 500 而高頻重試；這通常表示模型與請求協定不相容。先確認該模型的 `supported_endpoint_types` 是否包含 `openai`，再選擇相符的介面。
