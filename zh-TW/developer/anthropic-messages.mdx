...
OpenAI 相容應用程式應優先使用 [Chat Completions](/zh-TW/developer/openai-compatible)。若模型的 `supported_endpoint_types` 未包含 `anthropic`，不要將 Messages 當成通用介面；`not implemented` 通常表示協定不相容，並非重試可解決的暫時錯誤。
</Warning>
