...
在控制台的 **令牌管理 → 新增令牌** 建立 Key，選取包含 Claude Code 所需模型的群組並設定合理上限。請以模型廣場或[能力矩陣](/zh-TW/developer/capability-matrix)確認模型與協定。
...
如果錯誤明確提到不支援 `context_management` 或實驗欄位，才在 `env` 中加入 `CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1` 後測試。此設定會關閉部分預發布欄位，僅在這類相容性錯誤出現時使用即可。
</Tip>
