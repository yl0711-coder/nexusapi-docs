...
Use [Chat Completions](/en/developer/openai-compatible) for OpenAI-compatible applications. If `supported_endpoint_types` does not include `anthropic`, do not treat Messages as a universal endpoint; `not implemented` normally means a protocol mismatch, not a temporary error that retries can solve.
</Warning>
