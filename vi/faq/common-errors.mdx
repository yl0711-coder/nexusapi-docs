...
Nếu lỗi có cả `status_code=500` và `not implemented`, đồng thời `supported_endpoint_types` của model đã chọn không chứa `openai-response` nhưng yêu cầu dùng `/v1/responses`, đó là **không tương thích giao thức**, không phải sự cố upstream sẽ tự hết khi chờ.

1. Ứng dụng tự xây dựng chọn endpoint theo `supported_endpoint_types`; khi có `openai`, dùng `/v1/chat/completions`.
2. Với Claude Code, làm theo [hướng dẫn Claude Code](/vi/tools/claude-code) và xác nhận model hỗ trợ `anthropic`.
...
Model không khả dụng thường không phải 404. Model không thuộc nhóm key, tạm thời không khả dụng hoặc không có dịch vụ khả dụng thường trả `503` / `no available channel`.
...
Điều này nghĩa là nhóm của Key hiện tại tạm thời không có dịch vụ khả dụng cho model đó. ID sai, model không có trong nhóm, model tạm thời không khả dụng hoặc đang bảo trì đều có thể gây ra.
